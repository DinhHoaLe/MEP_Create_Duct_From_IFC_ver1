...
namespace IFCInfo
{
    public sealed class LinkOption { public long Id { get; set; } public string Name { get; set; } public string IfcPath { get; set; } public bool IsLoaded { get; set; } public override string ToString() { return Name + (IsLoaded ? "" : " (chưa load)"); } }
    ...
    public sealed class IFCInfoWindow : Window
    {
        ...
        public LinkOption SelectedLink { get; private set; }
        public Action<LinkOption> LoadLink { get; set; }
        ...
        public IFCInfoWindow()
        {
            ...
            titles.Children.Add(heading); titles.Children.Add(subtitle);
            ...
            var info = Text("ⓘ", 24, "#7A96BA"); info.Margin = new Thickness(0,0,14,0); status.Children.Add(info);
            feedback = Text("Chọn IFC link và Category để tiếp tục.", 14, "#647FA6"); feedback.MaxWidth = 310; status.Children.Add(feedback); footer.Children.Add(status);
            ...
            footer.SizeChanged += (s, e) => {
                ...
                Grid.SetRow(buttons, compact ? 1 : 0); Grid.SetColumn(buttons, compact ? 0 : 1); Grid.SetColumnSpan(buttons, compact ? 2 : 1);
                Grid.SetColumnSpan(status, compact ? 2 : 1);
            };
            var back = Button("← Quay lại", false); back.Visibility = Visibility.Collapsed; buttons.Children.Add(back);
            var copy = Button("Sao chép bảng", false); copy.Visibility = Visibility.Collapsed; buttons.Children.Add(copy);
            copy.Click += (s, e) => Copy(SystemTableText());
            var next = Button("Tiếp tục →", true); next.IsEnabled = false; buttons.Children.Add(next);
            var replace = Button("Thay thế Air Terminal →", true); replace.Visibility = Visibility.Collapsed; buttons.Children.Add(replace);
            var create = Button("Tạo Duct →", true); create.Visibility = Visibility.Collapsed; buttons.Children.Add(create);
            var close = Button("Đóng", false); close.IsCancel = true; close.Click += (s, e) => Close(); buttons.Children.Add(close);
            var body = new StackPanel { Margin = new Thickness(38, 6, 38, 20) };
            var card = new StackPanel { Margin = new Thickness(34, 25, 34, 32) };
            body.Children.Add(new Border { Child = card, Background = Brushes.White, CornerRadius = new CornerRadius(16),
                BorderBrush = Brush("#E0EAF7"), BorderThickness = new Thickness(1), Effect = UiDesign.Shadow() });
            var sourceTitle = Text("Thông tin nguồn", 22, "#102A50"); sourceTitle.FontWeight = FontWeights.SemiBold; card.Children.Add(sourceTitle);
            var description = Text("Chọn IFC link và Category để tạo MEP từ mô hình IFC.", 15, "#647FA6");
            description.Margin = new Thickness(0,8,0,0); card.Children.Add(description);
            ...
            linkBox.SelectionChanged += (s, e) => {
                next.IsEnabled = false; categoryBox.ItemsSource = null; categoryBox.IsEnabled = false;
                SelectedLink = linkBox.SelectedItem as LinkOption;
                SelectedCategory = null; Replacement = null; DuctCreationRequest = null;
                Categories = new List<CategoryOption>(); AirTerminals = new List<AirTerminalRow>();
                LoadCategory = null; PrepareDucts = null; CanCreateDucts = false; CanReplaceCategory = false;
                if (SelectedLink == null) return;
                if (!SelectedLink.IsLoaded) { feedback.Text = "Link chưa được load. Hãy load link trong Revit rồi mở lại tool."; return; }
                try {
            ...
            next.Click += (s, e) => {
                ...
                if (SelectedCategory == null || LoadCategory == null) return;
                try { LoadCategory(SelectedCategory); }
                ...
                scroll.Content = CountPage(); scroll.ScrollToTop();
                next.Visibility = Visibility.Collapsed; back.Visibility = Visibility.Visible; copy.Visibility = Visibility.Visible;
            ...
            back.Click += (s, e) => {
                heading.Text = "Chọn link và Category"; subtitle.Text = "Bước 1 · Chọn nguồn để tiếp tục";
                progress.Content = UiDesign.Steps(1);
                scroll.Content = body; scroll.ScrollToTop();
                next.Visibility = Visibility.Visible; back.Visibility = copy.Visibility = replace.Visibility = create.Visibility = Visibility.Collapsed;
                feedback.Text = "Chọn IFC link và Category.";
            };
            replace.Click += (s, e) => {
                var selected = AirTerminals.Where(row => row.IsSelected).ToList();
                if (selected.Count == 0) { feedback.Text = "Hãy tích chọn ít nhất một Air Terminal."; return; }
            ...
            create.Click += (s, e) => {
                var selected = AirTerminals.Where(row => row.IsSelected).ToList();
                if (selected.Count == 0) { feedback.Text = "Hãy tích chọn ít nhất một ống."; return; }
                try { var request = PrepareDucts?.Invoke(selected); if (request != null) { DuctCreationRequest = request; Close(); } }
        ...
        private FrameworkElement CountPage()
        {
            ...
            if (CanCreateDucts)
            {
                ...
                exists.Setters.Add(new Setter(TextBlock.ForegroundProperty, Brush("#16815D"))); statusStyle.Triggers.Add(exists);
                var missing = new DataTrigger { Binding = new System.Windows.Data.Binding("DuctExistence"), Value = "Chưa tồn tại" };
                missing.Setters.Add(new Setter(TextBlock.ForegroundProperty, Brush("#637FA5"))); statusStyle.Triggers.Add(missing);
                table.Columns.Add(new DataGridTextColumn { Header = "Duct đã tồn tại", Width = 155, IsReadOnly = true,
                    Binding = new System.Windows.Data.Binding("DuctExistence"), ElementStyle = statusStyle });
            }
        ...
        public string SystemTableText()
        {
            ...
            {
                result.AppendLine();
                if (CanCreateDucts) result.Append(CleanCell(row.DuctExistence) + "\t");
        ...
        {
            var button = new Button { Content = caption, Padding = new Thickness(24, 13, 24, 13), FontSize = 16, FontWeight = FontWeights.SemiBold, MinHeight = 48,
