...
namespace IFCInfo
{
    internal static class AirTerminalReplacement
    {
        ...
        public static void Configure(IFCInfoWindow window, Document doc)
        {
            using (var types = new FilteredElementCollector(doc))
            {
                window.ReplacementTypes = types.OfClass(typeof(FamilySymbol))
                    ...
                    .Select(symbol =>
                    {
                        ...
                        return new ReplacementTypeOption { Id = symbol.Id.Value,
                            Label = symbol.Family.Name + " : " + symbol.Name,
                            Supported = supported, NeedsHost = host,
            ...
            using (var levels = new FilteredElementCollector(doc))
                window.ReplacementLevels = levels.OfClass(typeof(Level)).Cast<Level>()
                    ...
                    { Id = level.Id.Value, Label = level.Name }).ToList();
        }
        ...
        public static void Execute(UIDocument uiDoc, RevitLinkInstance link, IFCInfoWindow window)
        {
            ...
            if (linkedDoc == null) throw new InvalidOperationException("Link chưa được load.");
            if (doc.IsReadOnly || doc.IsFamilyDocument) throw new InvalidOperationException("Cần một project Revit có thể chỉnh sửa.");
            ...
            if (symbol == null || level == null) throw new InvalidOperationException("Family/Type hoặc Level không còn tồn tại.");
            FamilyPlacementType placement = symbol.Family.FamilyPlacementType;
            if (placement != FamilyPlacementType.OneLevelBased && placement != FamilyPlacementType.OneLevelBasedHosted &&
                placement != FamilyPlacementType.WorkPlaneBased) throw new InvalidOperationException("Kiểu family chưa hỗ trợ.");
            ...
            if (schema != null)
            {
                using (var existing = new FilteredElementCollector(doc))
                {
                    foreach (Element element in existing.OfClass(typeof(FamilyInstance)).OfCategory(BuiltInCategory.OST_DuctTerminal))
                    {
                        ...
                        if (entity.IsValid()) alreadyCreated.Add(entity.Get<string>(schema.GetField("SourceKey")));
                    }
                }
            }
            int skipped = 0;
            ...
            foreach (string sourceId in request.SourceIds.Distinct())
            {
                ...
                if (alreadyCreated.Contains(key)) { skipped++; continue; }
                XYZ localPoint;
                var location = source.Location as LocationPoint;
                if (location != null) localPoint = location.Point;
                else
                {
                    BoundingBoxXYZ box = source.get_BoundingBox(null);
                    if (box == null) throw new InvalidOperationException("Không có vị trí/hình học của phần tử " + sourceId);
                    localPoint = box.Transform.OfPoint((box.Min + box.Max) * 0.5);
                    boundingCenters++;
                }
                items.Add(new PlacementItem { Point = linkTransform.OfPoint(localPoint), Key = key,
                    Row = sourceRow });
            }
            if (items.Count == 0)
            {
            ...
            if (placement != FamilyPlacementType.OneLevelBased)
            {
                ...
                if (!(face is PlanarFace)) throw new InvalidOperationException("Bản test hỗ trợ mặt host phẳng.");
            ...
            using (var transaction = new Transaction(doc, "IFC Info - Create replacement Air Terminals"))
            {
                ...
                try
                {
                    ...
                    foreach (PlacementItem item in items)
                    {
                        ...
                        else instance = doc.Create.NewFamilyInstance(item.Point, symbol, level, StructuralType.NonStructural);
                        if (instance == null) throw new InvalidOperationException("Revit không tạo được family cho " + item.Row.ElementId);
                        doc.Regenerate();
                        if (placement == FamilyPlacementType.OneLevelBased)
                        {
                            var actual = instance.Location as LocationPoint;
                            if (actual == null) throw new InvalidOperationException("Family mới không có LocationPoint.");
                            XYZ delta = item.Point - actual.Point;
                            if (delta.GetLength() > 1e-7) ElementTransformUtils.MoveElement(doc, instance.Id, delta);
                        }
        ...
        private sealed class HostFaceFilter : ISelectionFilter
        {
            ...
            public HostFaceFilter(Document doc) { this.doc = doc; }
            public bool AllowElement(Element element) { return !(element is RevitLinkInstance); }
