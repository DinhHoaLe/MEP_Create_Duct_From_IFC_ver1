    ...
    internal static class DuctCreation
    {
        ...
        public static void Configure(IFCInfoWindow window, Document doc, RevitLinkInstance link)
        {
            ...
            window.LoadCategory = category =>
            {
                ...
                if (window.CanCreateDucts) UpdateExistence(window, doc, link);
            };
        ...
        private static void UpdateExistence(IFCInfoWindow window, Document doc, RevitLinkInstance link)
        {
            ...
            foreach (Duct duct in new FilteredElementCollector(doc).OfClass(typeof(Duct)))
            {
                ...
                if (line == null) { incomplete = true; continue; }
                ducts.Add(new DuctCoverage.Segment { Id = duct.Id.Value,
                    Start = Coordinates(line.GetEndPoint(0)), End = Coordinates(line.GetEndPoint(1)) });
            }
            Transform transform = link.GetTotalTransform();
            foreach (var row in window.AirTerminals)
            {
                ...
                {
                    Element source = link.GetLinkDocument()?.GetElement(new ElementId(long.Parse(row.ElementId)));
                    if (source == null) throw new InvalidOperationException("Không tìm thấy ống nguồn.");
                    var geometry = ReadGeometry(source, row.DuctSource);
