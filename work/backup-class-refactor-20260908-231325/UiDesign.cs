...
namespace IFCInfo
{
    internal static class UiDesign
    {
        ...
        internal static DropShadowEffect Shadow(double opacity = .08) => new DropShadowEffect {
            Color = Color.FromRgb(37,87,151), BlurRadius = 22, ShadowDepth = 4, Opacity = opacity };
...
<Grid xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' Width='88' Height='94'>
 ...
 <Border Background='#137CFA' CornerRadius='5' HorizontalAlignment='Right' VerticalAlignment='Bottom' Padding='10,3' Margin='0,0,0,3'><TextBlock Text='IFC' FontSize='18' FontWeight='SemiBold' Foreground='White'/></Border>
        ...
        {
            var tile = new Border { Width = 50, Height = 50, CornerRadius = new CornerRadius(10),
                Background = IFCInfoWindow.Brush(category ? "#F0EEFF" : "#EAF3FF"), VerticalAlignment = VerticalAlignment.Top };
            var path = new System.Windows.Shapes.Path { Data = Geometry.Parse(category
                ...
                Stroke = IFCInfoWindow.Brush(category ? "#4D42E8" : "#147BFA"), StrokeThickness = 2.3,
                StrokeStartLineCap = PenLineCap.Round, StrokeEndLineCap = PenLineCap.Round,
                Stretch = Stretch.Uniform, Width = 24, Height = 24 };
            tile.Child = path; return tile;
        }
        internal static FrameworkElement Steps(int active)
        {
            var grid = new Grid { Width = 340, VerticalAlignment = VerticalAlignment.Center };
            for (int i = 0; i < 5; i++) grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(i % 2 == 0 ? 90 : 35) });
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                var number = new Border { Width = 35, Height = 35, CornerRadius = new CornerRadius(18),
                    Background = IFCInfoWindow.Brush(current ? "#147BFA" : done ? "#E0EEFF" : "#E8EFF8"),
                    BorderBrush = IFCInfoWindow.Brush(current ? "#147BFA" : "#D6E3F3"), BorderThickness = new Thickness(1) };
                number.Child = new TextBlock { Text = done ? "✓" : (i + 1).ToString(), FontSize = 17, FontWeight = FontWeights.SemiBold,
                    HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center,
                    Foreground = IFCInfoWindow.Brush(current ? "#FFFFFF" : "#5D789F") };
                panel.Children.Add(number);
                panel.Children.Add(new TextBlock { Text = labels[i], TextAlignment = TextAlignment.Center, FontSize = 12,
                    Margin = new Thickness(0,8,0,0), Foreground = IFCInfoWindow.Brush(current ? "#0872F4" : "#617CA1"),
                    FontWeight = current ? FontWeights.SemiBold : FontWeights.Normal });
                Grid.SetColumn(panel, i * 2); grid.Children.Add(panel);
                if (i < 2) { var line = new Border { Height = 2, Background = IFCInfoWindow.Brush(i + 1 < active ? "#147BFA" : "#CEDCF0"),
                    VerticalAlignment = VerticalAlignment.Top, Margin = new Thickness(-9,17,-9,0) }; Grid.SetColumn(line, i * 2 + 1); grid.Children.Add(line); }
            }
            return grid;
        }
        internal static ComboBox Field(StackPanel body, string title, string description, string placeholder, bool category)
        {
            ...
            var field = new StackPanel(); Grid.SetColumn(field, 1); grid.Children.Add(field);
            var label = IFCInfoWindow.Text(title, 17, "#102A50"); label.FontWeight = FontWeights.SemiBold; field.Children.Add(label);
            var hint = IFCInfoWindow.Text(description, 14, "#637FA5"); hint.Margin = new Thickness(0,6,0,12); field.Children.Add(hint);
            var combo = new ComboBox { MinHeight = 46, FontSize = 16, Tag = placeholder, Padding = new Thickness(15,10,40,10),
                Foreground = IFCInfoWindow.Brush("#183459"), HorizontalContentAlignment = HorizontalAlignment.Stretch };
...
<Style xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' TargetType='ComboBox'>
 ...
 <Setter Property='ItemContainerStyle'><Setter.Value><Style TargetType='ComboBoxItem'><Setter Property='Padding' Value='12,9'/><Setter Property='HorizontalContentAlignment' Value='Stretch'/></Style></Setter.Value></Setter>
            ...
            field.Children.Add(combo); body.Children.Add(grid); return combo;
        }
    }
}
