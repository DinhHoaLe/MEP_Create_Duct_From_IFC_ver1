...
namespace IFCInfo
{
    ...
    {
        public sealed class Segment { public long Id; public double[] Start, End; }
        public sealed class Result { public string Status; public List<long> Ids = new List<long>(); }
        public static Result Check(double[] start, double[] end, IEnumerable<Segment> ducts, double tolerance)
        {
            ...
            if (length <= tolerance) return new Result { Status = "Chưa xác định" };
            var axis = vector.Select(v => v / length).ToArray();
            var intervals = new List<double[]>();
            var result = new Result();
            foreach (var duct in ducts)
            {
                var a = Sub(duct.Start, start); var b = Sub(duct.End, start);
                double t0 = Dot(a, axis), t1 = Dot(b, axis);
                if (DistanceToAxis(a, axis, t0) > tolerance || DistanceToAxis(b, axis, t1) > tolerance) continue;
                double low = Math.Max(0, Math.Min(t0, t1)), high = Math.Min(length, Math.Max(t0, t1));
                if (high - low <= tolerance) continue;
                intervals.Add(new[] { low, high }); result.Ids.Add(duct.Id);
            }
            if (intervals.Count == 0) { result.Status = "Chưa tồn tại"; return result; }
            double covered = 0;
            foreach (var span in intervals.OrderBy(i => i[0]))
            {
                if (span[0] > covered + tolerance) { result.Status = "Trùng một phần"; return result; }
