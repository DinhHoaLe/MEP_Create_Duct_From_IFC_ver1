...
namespace IFCInfo
{
    ...
    public sealed class AirTerminalReplacementWindow : Window
    {
            ...
            List<ReplacementTypeOption> types, List<ReplacementLevelOption> levels)
        {
            ...
            Width = 780; Height = 700; MinWidth = 580; MinHeight = 460;
            ...
            title.Margin = new Thickness(0, 8, 0, 0); head.Children.Add(title);
            header.Child = head; DockPanel.SetDock(header, Dock.Top); root.Children.Add(header);
            ...
            var cancel = IFCInfoWindow.Button("Quay lại", false); cancel.IsCancel = true;
            cancel.Click += (s, e) => Close(); buttons.Children.Add(cancel);
            var create = IFCInfoWindow.Button("Tạo " + rows.Count + " Air Terminals", true);
            buttons.Children.Add(create); footerStack.Children.Add(buttons);
            footer.Child = footerStack; DockPanel.SetDock(footer, Dock.Bottom); root.Children.Add(footer);

            var scroll = new ScrollViewer { VerticalScrollBarVisibility = ScrollBarVisibility.Auto };
            root.Children.Add(scroll);
            var body = new StackPanel { Margin = new Thickness(24) }; scroll.Content = body;
            ...
            var typeBox = new ComboBox { ItemsSource = types, MinHeight = 36, IsTextSearchEnabled = true,
                HorizontalContentAlignment = HorizontalAlignment.Stretch };
            body.Children.Add(typeBox);
            var placement = IFCInfoWindow.Text("Chọn type đã load trong model Revit chính.", 12, "#526880");
            placement.Margin = new Thickness(0, 8, 0, 0); body.Children.Add(placement);
            ...
            if (levels.Count > 0) levelBox.SelectedIndex = 0;
            ...
            notes.Margin = new Thickness(0, 20, 0, 0); body.Children.Add(notes);
            Action validate = () =>
            {
            ...
            create.Click += (s, e) =>
            {
                ...
                { status.Text = "Nhập góc xoay hợp lệ từ -360 đến 360 độ."; return; }
                var type = typeBox.SelectedItem as ReplacementTypeOption;
                var level = levelBox.SelectedItem as ReplacementLevelOption;
                if (type == null || !type.Supported || level == null) return;
                Request = new ReplacementRequest { TypeId = type.Id, LevelId = level.Id,
                    RotationDegrees = degrees, SourceIds = rows.Select(row => row.ElementId).Distinct().ToList() };
                DialogResult = true;
            };
        }
        private static void Label(StackPanel body, string label)
        {
            ...
            text.Margin = new Thickness(0, 20, 0, 8); body.Children.Add(text);
        }
    }
}
