...
namespace IFCInfo
{
    ...
    public class IFCInfoCommand : IExternalCommand
    {
            ...
            ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                foreach (var link in links)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(original) && !ifcPath && !ifcName) continue;
                    string sourcePath = string.IsNullOrWhiteSpace(original) ? path : original.Trim();
                    if (sourcePath.EndsWith(".ifc.rvt", StringComparison.OrdinalIgnoreCase)) sourcePath = sourcePath.Substring(0, sourcePath.Length - 4);
                    dialog.Links.Add(new LinkOption { Id = link.Id.Value, Name = type.Name, IfcPath = sourcePath, IsLoaded = linkedDoc != null });
                }
                foreach (var group in dialog.Links.GroupBy(l => l.Name).Where(g => g.Count() > 1))
                {
                    int number = 0;
                    foreach (var item in group.OrderBy(l => l.Id)) item.Name += " · Bản " + (++number);
                }
                dialog.Links = dialog.Links.OrderBy(l => l.Name).ToList();
                ...
                dialog.LoadLink = option => {
                    ...
                    if (link?.GetLinkDocument() == null) throw new InvalidOperationException("Link chưa được load.");
                    ConfigureCategories(dialog, link.GetLinkDocument(), option.IfcPath);
                    DuctCreation.Configure(dialog, doc, link);
                    selectedLink = link;
                };
        ...
        private static void ConfigureCategories(IFCInfoWindow window, Document linkedDoc, string ifcPath)
        {
        ...
        private static void SetCategoryCount(IFCInfoWindow window, Document linkedDoc, string ifcPath, CategoryOption category)
        {
            ...
            try
            {
                ...
                else if (readIfc) window.IfcSourceStatus = "Chưa chọn IFC gốc. Chỉ đọc parameter trong Revit; chưa có cao độ IFC.";
                ...
                using (var collector = new FilteredElementCollector(linkedDoc))
                {
                    ...
                    foreach (Element element in terminals)
                    {
                        ...
                        {
                            matched++;
                            if (isDuct) row.DuctSource = sourceRow;
