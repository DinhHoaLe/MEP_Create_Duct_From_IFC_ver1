    ...
    public class IFCInfoCommand : IExternalCommand
    {
        ...
        private static void SetCategoryCount(IFCInfoWindow window, Document linkedDoc, string ifcPath, CategoryOption category)
        {
            ...
            try
            {
                ...
                if (window.CanReplaceCategory && !File.Exists(ifcPath))
                {
                ...
                else if (window.CanReplaceCategory) window.IfcSourceStatus = "Chưa chọn IFC gốc. Chỉ đọc parameter trong Revit; chưa có cao độ IFC.";
                int matched = 0;
